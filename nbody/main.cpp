...
public:
	...
	void setCam();
	//Draw orbits based on current settings
...
public:
	...
	//Reset camera
	void OnKeyboard();
	//Apply the camera matrix and move the view according to camera's angle and position
	void look();
	//Destructor
	~Camera();
	//Get angle Y
	...
	//Is camera set to lock
	bool camFollow;
protected:
	//Sensitivity
	double _motionSensivity;
	//Distance from the locked on object
	double _distance;
	//Angles
	double _angleY;
	double _angleZ;
...
void initTexture(std::string name, int cnt);
		...
		{
			{
				...
				if (gContext == NULL)
				{
					SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "OpenGL context could not be created!", SDL_GetError(), gWindow);
				...
				else
				{
					...
					{
						success = false;
						SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error initializing GLEW!", SDL_GetError(), gWindow);
					...
					{
						success = false;
						SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Warning: Unable to set VSync!", SDL_GetError(), gWindow);
						//printf("Warning: Unable to set VSync! SDL Error: %s\n", SDL_GetError());
					}
					//Initialize OpenGL
					if (!initGL())
					{
						SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Unable to initialize OpenGL!", SDL_GetError(), gWindow);
...
void Body::draw(float rotate)
{
	...
	if (id == 0)
	{
		glMaterialfv(GL_FRONT, GL_EMISSION, sunLight);
	}
	else
	{
		glMaterialfv(GL_FRONT, GL_EMISSION, otherLight);
	}
	glPushMatrix();
...
void mainLoop()
{
	...
	for (int i = 0; i < 11; ++i)
	{
		initTexture(bodies[i].getName(), i);
	}
	initTexture("stars", 11);
	//Main loop
	while (!handleInput())
	{
		...
		ImGui::Separator();
		ImGui::SliderFloat("Секунд в шаг", &step, 1.f, 86400.0f);
